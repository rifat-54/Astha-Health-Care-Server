...
const formatErrorMeta=(meta?:Record<string,unknown>)=>{
    if(!meta) return ""

    const parts:string[]=[]

    if(meta.target){
        parts.push(`Fields:${String(meta.target)}`)
    }
...
export const handlePrismaClientKnownRequestError=(error:Prisma.PrismaClientKnownRequestError)=>{
    ...
    // split by new line and take the first line as the4 main message,rest can be added to error  source

    const lines=cleanMessage.split("\n").filter(line=>line.trim())
    const mainMessage=lines[0] || "An error occured with the database operation"
...
export const handlePrismaClientUnknownRequestError=(error:Prisma.PrismaClientUnknownRequestError)=>{
    ...
    const lines=cleanMessage.split("\n").filter(line=>line.trim())
    const mainMessage=lines[0] || "An unknown error occured with the database operation"
